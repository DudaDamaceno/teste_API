...
class API {
  ...
  static BaseOptions opts = BaseOptions( //config base de onde vc vai requisitar os dados
      // baseUrl: ,
      responseType: ResponseType.json,
      connectTimeout: 30000, //tempo de conexão (30s)
  ...
  static Dio addInterceptors(Dio dio) { //parte de interceptação
    ...
    switch (err.type) {
      ...
      case DioErrorType.response:
        switch (err.response?.statusCode) {
          case 400:
            throw BadRequestException(err.requestOptions); //throw = retorno, mais usado que o return
          case 401:
  ...
  Future<Response?> get(String url, {dynamic data, Options? options}) async {
    try { //tudo q estiver dentro do try ele vai tentar fazer, o que der erro vai pro catch
      Response response =
